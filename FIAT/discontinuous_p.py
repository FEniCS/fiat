...
class DiscontinuousPDualSet(dual_set.DualSet):
    ...
    def __init__(self, ref_el, degree):
        ...
        super(DiscontinuousPDualSet, self).__init__(nodes, hypercube_simplex_map[ref_el], entity_ids)


class HigherOrderDiscontinuousP(finite_element.CiarletElement):
    ...
    def __init__(self, ref_el, degree):
        ...
        dual = DiscontinuousPDualSet(ref_el, degree)
        formdegree = ref_el.get_spatial_dimension()  # n-form
        super(HigherOrderDiscontinuousP, self).__init__(poly_set=poly_set,
                                                                  dual=dual,
                                                                  order=degree,
                                                                  ref_el=ref_el,
                                                                  formdegree=formdegree)
...
def DiscontinuousP(ref_el, degree):
    if degree == 0:
        return DPC0(ref_el)
    else:
        return HigherOrderDiscontinuousP(ref_el, degree)
